...
if (!args[0]) {
    ...
    if (message.guild.channels.find(c => c.name.toLowerCase() === '-= tickets =-')) {
        ...
        c.overwritePermissions(message.guild.roles.find(r => r.name.toLowerCase() === 'support'), {
            VIEW_CHANNEL: true
        })
...
fs.writeFile('./tickets.json', '{\n"id":' + data.id + "\n}", (err) => {
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send(new Discord.RichEmbed().setColor(Config.ticketcolor).setAuthor(message.author.tag, client.user.displayAvatarURL).setTimestamp().setDescription("Je hebt hiervoor geen permissie!"));
    if (message.channel.name.startsWith("ticket-")) {
        message.channel.delete();
    } else {
        message.channel.send(new Discord.RichEmbed().setColor(Config.embedColor).setAuthor(message.author.tag, client.user.displayAvatarURL).setTimestamp().setDescription(message.author + ', deze command kan enkel worden gebruikt in een ticket.'))
        return
    }
    }
    if (args[0] === 'remove') {
